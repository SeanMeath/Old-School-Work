<!DOCTYPE html>
<html>

<head>
        <link rel="stylesheet" type="text/css" href="style.css">
        <title>:) Recipes</title>
</head>

<body>
        <nav>
        <ul id="topNav">
                <li><a class="active" href="Recipes_HomePage.php">:) Recipes</a></li>
                <li><a href="Recipes_Categories.php">Categories</a></li>
                <li><a href="Recipes_Recipe.php">Random Recipe</a></li>
                <li style="float:right">
                    <form action="Recipes_Search.php" method="GET">
                            <input type="text" id="searchBar" name="Search" placeholder="Search...">
                            <input type="submit" id="searchButton" value="Search">
                    </form>
                </li>
        </ul>
        </nav>

        <div id="mainPage">
        <?php
        require "Recipes_Connect.php";
        $conn = $GLOBALS["conn"];

        if($conn){

            // OVEN
            echo '<form id="convert" action="Recipes_HomePage.php" method="POST">';
            echo '<h2>Temperature Converter</h2>';
            echo '<ul>';
            echo '<li><h3>Num<input type="text" name="NUM"></li>';
            echo '<li><h3>From: <select name="FROM">';
            echo '<option value="C">Celcius</option>';
            echo '<option value="F">Fareinheight</option>';
            echo '<option value="G">Gas Mark</option>';
            echo '</select></h3></li>';
            echo '<li><h3>To: <select name="TO">';
            echo '<option value="C">Celcius</option>';
            echo '<option value="F">Fareinheight</option>';
            echo '<option value="G">Gas Mark</option>';
            echo '</select></h3></li>';
            echo '<li><input type="submit" value="Submit"></li></ul>';
            echo '<br>';

            if(isset($_POST["NUM"])){
                $stid = oci_parse($conn, "BEGIN :bvRet := ConvertOvenTemp(:bvRat, :bvFrom, :bvTo); END;");
                oci_bind_by_name($stid, ":bvRat", $_POST["NUM"], 32);
                oci_bind_by_name($stid, ":bvFrom", $_POST["FROM"], 32);
                oci_bind_by_name($stid, ":bvTo", $_POST["TO"], 32);
                oci_bind_by_name($stid, ":bvRet", $ret, 32);
                oci_execute($stid);
                echo '<h1>Converted Temp: ' . $ret . '</h1>';
                echo '<br>';
            }

            echo '</form>';

            echo "<h1>Quick Recipes:</h1>";
            $stid = oci_parse($conn, "BEGIN GetQuick(:bvCurs); END;");
            $curRecList = oci_new_cursor($conn);
            oci_bind_by_name($stid, ":bvCurs", $curRecList, -1, OCI_B_CURSOR);
            oci_execute($stid);
            oci_execute($curRecList);

            echo '<ul class="striped-list">';
            while(($row = oci_fetch_assoc($curRecList)) != false){
                echo "<li class='recipeTile'><a href='Recipes_Recipe.php?ID=" . $row['RECID'] . "'>";
                echo '<img src="Images/' . str_pad($row['RECID'], 3, "0", STR_PAD_LEFT) . '.png" alt="ERROR">';
                echo "<h1>" . $row['RECNAME'] . "</h1>";
                echo "<p>" . "Description: " . $row['RECDESC'] . "</p>";
                echo "<p>" . "PrepTime: " . $row['PREPTIME'] . " Minutes</p>";
                echo "<p>" . "CookTime: " . $row['COOKTIME'] . " Minutes</p>";

                $numStars = floor($row['AVERAGERATING']);
                echo '<p>Average Rating: ';
                for($i = 0; $i < $numStars; $i++){
                        echo '&#9733;';
                }
                echo '</p>';

                echo "</a></li>";
            }
            echo '</ul>';

            echo "<h1>Easy Recipes:</h1>";
            $stid = oci_parse($conn, "BEGIN GetEasy(:bvCurs); END;");
            $curRecList = oci_new_cursor($conn);
            oci_bind_by_name($stid, ":bvCurs", $curRecList, -1, OCI_B_CURSOR);
            oci_execute($stid);
            oci_execute($curRecList);

            echo '<ul class="striped-list">';
            while(($row = oci_fetch_assoc($curRecList)) != false){
                echo "<li class='recipeTile'><a href='Recipes_Recipe.php?ID=" . $row['RECID'] . "'>";
                echo '<img src="Images/' . str_pad($row['RECID'], 3, "0", STR_PAD_LEFT) . '.png" alt="ERROR">';
                echo "<h1>" . $row['RECNAME'] . "</h1>";
                echo "<p>" . "Description: " . $row['RECDESC'] . "</p>";
                echo "<p>" . "PrepTime: " . $row['PREPTIME'] . " Minutes</p>";
                echo "<p>" . "CookTime: " . $row['COOKTIME'] . " Minutes</p>";

                $numStars = floor($row['AVERAGERATING']);
                echo '<p>Average Rating: ';
                for($i = 0; $i < $numStars; $i++){
                        echo '&#9733;';
                }
                echo '</p>';

                echo "</a></li>";
            }
            echo '</ul>';
        }
        ?>
        </div>
</body>

</html>