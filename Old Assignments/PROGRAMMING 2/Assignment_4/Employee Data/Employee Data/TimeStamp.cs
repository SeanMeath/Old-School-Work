...
namespace Employee_Data
{
    class TimeStamp
    {
        ...
        public static TimeStamp ReadFromString(string tsData)
        {
            string[] info = tsData.Split(':');
            int ARRAYSIZE = info.Length;
            int[] times = new int[ARRAYSIZE];
            for (int i = 0; i < ARRAYSIZE; i++)
            {
                times[i] = int.Parse(info[i]);
            }
            TimeStamp ts = new TimeStamp(times[0], times[1], times[2]);
            return ts;
        }
        #endregion
        ...
        #region Times
        public static decimal operator *(TimeStamp ts, decimal payRate)
        {
            decimal pay = (decimal)ts.ConvertToSeconds() / AMOUNTOFSECONDSIN_HOUR * payRate;
