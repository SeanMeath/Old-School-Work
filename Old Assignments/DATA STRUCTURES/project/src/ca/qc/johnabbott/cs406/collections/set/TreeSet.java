 ...
 * @author TODO
 */
public class TreeSet<T extends Comparable<T> > implements Set<T>, Serializable {
    ...
    public void writeTo(Serializer s) throws IOException {
        ...
        //Recursive call to read all nodes into the file
        writeTo(s, root);
    }

    public void writeTo(Serializer s, Node curr) throws IOException {
        //Base case
        if(curr == null){
            //Mark end of path
            s.serializeNull();
        ...
        s.serialize((Serializable) curr.element);
        //Recursive call to on left
        writeTo(s, curr.left);
        //Recursive call to on right
        writeTo(s, curr.right);
    }
    ...
    public void readFrom(Serializer s) throws IOException, SerializationException {
        ...
        //Recursive call to write all nodes into the tree
        root = deserializedAdd(root, s);
    }

    private Node<T> deserializedAdd(Node<T> curr, Serializer s) throws IOException, SerializationException{
        //Write the next part to the variable
        Serializable elem = s.deserialize();
        //If end of path, go back
        if(elem == null)
            return null;
        //Write the variable to the element
        T element = (T) elem;
        //Create new node
        curr = new Node<>(element);
        //Recursive call to on left
        curr.left = deserializedAdd(curr.left, s);
        //Recursive call to on right
        curr.right = deserializedAdd(curr.right, s);
