...
package ca.qc.johnabbott.cs616.noteapplication.sqlite;
...
public class TableFactory<T extends Identifiable<Long>> {
     ...
     * @param handler The SQLite handler of the database in which the table will be stored.
     * @param model The model class to use to construct the table.
     * @param <T>
     * @return
     */
    public static <T extends Identifiable<Long>> TableFactory<T> makeFactory(SQLiteOpenHelper handler, Class<T> model) {
        return new TableFactory<>(handler, model);
    }

    public static final SimpleDateFormat DEFAULT_DATE_FORMAT;

    static {
        DEFAULT_DATE_FORMAT = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ");
    }
    ...
    /**
     * Use the following date format instead of the default (yyyy-MM-dd'T'HH:mm:ss.SSSZ)
     * @param dateFormatString
     */
    public TableFactory<T> useDateFormat(String dateFormatString) {
    ...
    /**
     * Construct table.
     * @return The table class implementing
     */
                ...
                for (Method method : getSetters(model)) {
                    ...
                    try {
                        switch (type.getName()) {
                            ...
                            case "short":
                                method.invoke(element, cursor.getShort(pos));
                                break;

                            case "java.lang.Byte":
                            case "byte":
                                method.invoke(element, (byte) cursor.getInt(pos));
                                break;
                            ...
                            case "java.lang.Char":
                            case "char":
                                method.invoke(element, (char) cursor.getInt(pos));
                                break;
                ...
                for(Method method : getGetters(model)) {
                    ...
                    switch(method.getReturnType().getName()) {
                        ...
                        case "char":
                            ...
                            contentValues.put(columnName, (int) c);
                            break;
